...
class GovernanceClass(object):
    ...
    def vote(self, virclesd, signal, outcome):
        return self.go.vote(virclesd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, virclesd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(virclesd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(virclesd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import vircleslib
        obj_data = vircleslib.SHIM_serialise_for_virclesd(self.serialise())
    ...
    def virclesd_serialise(self):
        import vircleslib
        return vircleslib.SHIM_serialise_for_virclesd(self.serialise())
