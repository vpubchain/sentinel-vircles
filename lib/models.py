...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, virclesd):
        golist = virclesd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_virclesd(virclesd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_virclesd(self, virclesd, rec):
        import decimal
        import vircleslib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from virclesd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from virclesd, with every run
        ...
        # get/create, then sync payment amounts, etc. from virclesd - Virclesd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(virclesd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from virclesd! %s" % e)
            govobj.vote_delete(virclesd)
            return (govobj, None)
    ...
    def vote_delete(self, virclesd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(virclesd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, virclesd, signal, outcome):
        import vircleslib
        ...
        output = virclesd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = vircleslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(virclesd, signal)

    def sync_network_vote(self, virclesd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = virclesd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = VIRCLESD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 vircles addr, non-multisig
            if not vircleslib.is_valid_vircles_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Vircles address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Vircles Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (vircleslib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = VIRCLESD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not vircleslib.is_valid_vircles_address(addr, config.network):
    ...
    def hash(self):
        import vircleslib
        return vircleslib.hashit(self.serialise())
