...
class VirclesDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some virclesd methods
    ...
    @classmethod
    def from_vircles_conf(self, vircles_dot_conf):
        from vircles_config import VirclesConfig
        config_text = VirclesConfig.slurp_config_file(vircles_dot_conf)
        creds = VirclesConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = vircleslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = vircleslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = vircleslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_virclesd_proto_version_with_sentinel_ping)
