...
def is_valid_vircles_address(address, network='mainnet'):

    # bech32
    try:
        vircles_hrp = "tsys" if network == 'testnet' else "sys"
        witver, _ = segwit_addr.decode(vircles_hrp, address)
    ...
    # Support vircles address (T-address on testnet and S-address on mainnet)
    vircles_version = 65 if network == 'testnet' else 63
    ...
    try:
        ...
        # rescue from exception, not a valid Vircles address
        return False

    if (address_version != vircles_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.virclesd_serialise())
...
def SHIM_serialise_for_virclesd(sentinel_hex):
    ...
    # superblock => "trigger" in virclesd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # virclesd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    virclesd_hex = serialise(obj)
    return virclesd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('vircles.conf')
