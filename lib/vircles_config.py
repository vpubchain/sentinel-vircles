...
class VirclesConfig():
    ...
    def slurp_config_file(self, filename):
        # read vircles.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is vircles.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from vircles.conf
        ...
        default_port = 9380 if (network == 'mainnet') else 19370

        # use default port for network if not specified in vircles.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from vircles.conf
