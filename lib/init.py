...
def has_vircles_conf():
    import config
    import io

    valid_vircles_conf = False

    # ensure vircles_conf exists & readable
    #
    # if not, print a message stating that Vircles Core must be installed and
    # configured, including JSONRPC access in vircles.conf
    try:
        f = io.open(config.vircles_conf)
        valid_vircles_conf = True
    except IOError as e:
        print(e)

    return valid_vircles_conf
...
def main():
    ...
    if not has_vircles_conf():
        print("VirclesCore must be installed and configured, including JSONRPC access in vircles.conf")
        sys.exit(1)
