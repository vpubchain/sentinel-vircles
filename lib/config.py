...
sentinel_cfg = VirclesConfig.tokenize(sentinel_config_file)
sentinel_version = 1000000
min_virclesd_proto_version_with_sentinel_ping = 70015


def get_vircles_conf():
    if sys.platform == 'win32':
        vircles_conf = os.path.join(os.getenv('APPDATA'), "Vircles/vircles.conf")
    else:
        home = os.environ.get('HOME')

        vircles_conf = os.path.join(home, ".vircles/vircles.conf")
        if sys.platform == 'darwin':
            vircles_conf = os.path.join(home, "Library/Application Support/Vircles/vircles.conf")

    vircles_conf = sentinel_cfg.get('vircles_conf', vircles_conf)
