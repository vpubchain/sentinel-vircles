...
def test_valid_vircles_address():
    from vircleslib import is_valid_vircles_address

    main = valid_vircles_address()
    test = valid_vircles_address('testnet')
    main_btc = valid_bitcoin_address()
    test_btc = valid_bitcoin_address('testnet')

    assert is_valid_vircles_address(main) is True
    assert is_valid_vircles_address(main, 'mainnet') is True
    assert is_valid_vircles_address(main, 'testnet') is False

    assert is_valid_vircles_address(test) is False
    assert is_valid_vircles_address(test, 'mainnet') is False
    assert is_valid_vircles_address(test, 'testnet') is True

    assert is_valid_vircles_address(main_btc) is True
    assert is_valid_vircles_address(main_btc, 'mainnet') is True
    assert is_valid_vircles_address(main_btc, 'testnet') is False

    assert is_valid_vircles_address(test_btc) is False
    assert is_valid_vircles_address(test_btc, 'mainnet') is False
    assert is_valid_vircles_address(test_btc, 'testnet') is True


def test_invalid_vircles_address():
    from vircleslib import is_valid_vircles_address

    main = invalid_vircles_address()
    test = invalid_vircles_address('testnet')
    main_btc = invalid_bitcoin_address()
    test_btc = invalid_bitcoin_address('testnet')

    assert is_valid_vircles_address(main) is False
    assert is_valid_vircles_address(main, 'mainnet') is False
    assert is_valid_vircles_address(main, 'testnet') is False

    assert is_valid_vircles_address(test) is False
    assert is_valid_vircles_address(test, 'mainnet') is False
    assert is_valid_vircles_address(test, 'testnet') is False

    assert is_valid_vircles_address(main_btc) is False
    assert is_valid_vircles_address(main_btc, 'mainnet') is False
    assert is_valid_vircles_address(main_btc, 'testnet') is False

    assert is_valid_vircles_address(test_btc) is False
    assert is_valid_vircles_address(test_btc, 'mainnet') is False
    assert is_valid_vircles_address(test_btc, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from vircleslib import elect_mn
...
def test_parse_masternode_status_vin():
    from vircleslib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % vircleslib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(vircleslib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(vircleslib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(120).quantize(precision)
    assert int(vircleslib.blocks_to_seconds(16616)) == 996960
